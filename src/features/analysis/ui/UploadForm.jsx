...
export default function UploadForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // NAVEGAÇÃO MÁGICA: Vai para /monitor e envia o UUID do paciente junto!
      navigate('/monitor', { state: { activeUuid: responseData.uuid } });
      
    } catch (error) {
      alert("Erro de rede. Falha ao enviar o arquivo.");
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
        ...
        <div className="flex-1 w-full space-y-1">
          ...
          <input type="file" name="file" accept=".fastq,.fastq.gz,.fq,.fq.gz" required onChange={handleInputChange} disabled={!isDockerOnline || isUploading}
